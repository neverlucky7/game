...
{
	Mix_Music *m_pMusic;
	static CFileMgr<Mix_Music> c_filemgr;	// File Loader (path resolving and cached loading)

public:

	CSound();
	CSound(std::string filename);			// load sound from file
	CSound(Mix_Music* pMusic);				// attach sound directly from SDL_mixer Mix_Music structure
	~CSound(void);
	...
	void Attach(Mix_Music *pMusic);			// attach sound directly from SDL_mixer Mix_Music structure
	void Detach();

	// for internal / SDL_mixer use only
//	operator Mix_Music*()			{ return m_pMusic; }
